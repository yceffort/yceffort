<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&pause=1000&color=58A6FF&center=true&vCenter=true&width=500&lines=Grind.+Learn.+Repeat.)](https://yceffort.kr)

**Yongchan Kim** · Seoul

I build things for the web, write books about them, and break them down on my blog.

</div>

---

<table width="100%">
<tr>
<th align="left" width="50%">What I'm up to</th>
<th align="left" width="50%">Books I wrote</th>
</tr>
<tr>
<td valign="top">

- Leading frontend at **NAVER Financial**
- Shipping open-source tooling at [NaverPayDev](https://github.com/orgs/NaverPayDev/repositories)
- Blogging deep dives at [yceffort.kr](https://yceffort.kr)

</td>
<td valign="top">

- [**모던 리액트 Deep Dive**](https://wikibook.co.kr/react-deep-dive/) — React internals, dissected
- [**npm Deep Dive**](https://wikibook.co.kr/npm-deep-dive/) — The npm ecosystem, explained
- [**리액트 인터뷰 가이드**](https://wikibook.co.kr/react-interview-guide/) — Translated

</td>
</tr>
</table>

---

```javascript
const yceffort = {
  stack: ['React', 'Next.js', 'TypeScript', 'Node.js'],
  interests: ['Web Performance', 'Browser APIs', 'AI/LLM Tooling'],
  currently: 'Writing a book about Node.js internals',
};
```

<div align="center">

[![Blog](https://img.shields.io/badge/yceffort.kr-000?style=for-the-badge&logo=vercel&logoColor=white)](https://yceffort.kr)
[![Email](https://img.shields.io/badge/root@yceffort.kr-000?style=for-the-badge&logo=gmail&logoColor=white)](mailto:root@yceffort.kr)

</div>
